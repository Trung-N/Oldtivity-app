...
public class MyEvents extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        PostRequester post = new PostRequester(respHandler, "http://oldtivity.herokuapp.com/myevents", formBody);
        post.send();
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

    }

    private ArrayList<String> loadMyEvents() {
        final ArrayList<String> returnEvents = new ArrayList<>();
        userDatabase.child("events").addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                Iterator iterator = dataSnapshot.getChildren().iterator();
                while(iterator.hasNext()){
                    returnEvents.add(((DataSnapshot) iterator.next()).getKey());
                }
    ...
    private ArrayList<Event> constructEventArray(String events) throws JSONException {
        ...
        while (keys.hasNext()) {
            ...
            for(int i = 0; i < myEventIds.size(); i ++){
                ...
                System.out.println("IDIDIDIDIDID~~~~" + myEventIds.get(i));
